...
const simulation = d3.forceSimulation()
    .force('forceX', d3.forceX(d => x(d['release year'])))
    .force('forceCenter', d3.forceCenter(b_width/2, b_height/2))
...
d3.csv('data/netflix.csv').then(data=>{
    ...
    .force('forceX', d3.forceX().x(function(d) { return x(d['release year']); }))
    .force('forceCollide', d3.forceCollide(50).strength(0.2).radius(function(d) { return radius(d['user rating score']) })).on('tick', ticked);
    ...
    function overBubble(d){
        ...
        tooltip
            .style("left", d3.select(this).attr("cx") + 2 + "px")     
    ...
    function overArc(d){
        ...
        donut.selectAll('path').filter((dd, i) => dd.index == d.index).style('opacity', 0.1)
    ...
    function outOfArc(){
        ...
        // Part 2 - change opacity of an arc
        donut.selectAll('path').style('opacity', 2)
